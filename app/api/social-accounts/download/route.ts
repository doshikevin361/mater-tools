...
export async function GET(request: NextRequest) {
  try {
    ...
    const excelData = accounts.map((account, index) => ({
      ...
      "SMS Verified": account.twilioIntegration?.smsVerified ? "Yes" : "No",
      "Verification Code": account.twilioIntegration?.verificationCode || "",
      "Real Account": account.realAccount ? "Yes" : "No",
      "Twilio Integrated": account.twilioIntegration ? "Yes" : "No",
      "Profile URL": account.creationResult?.accountData?.profileUrl || "",
    ...
    const columnWidths = [
      ...
      { wch: 10 }, // Verified
      { wch: 12 }, // SMS Verified
      { wch: 15 }, // Verification Code
      { wch: 12 }, // Real Account
      { wch: 15 }, // Twilio Integrated
      { wch: 40 }, // Profile URL
    ...
    headers.set(
      "Content-Disposition",
      `attachment; filename="social-accounts-${platform}-${new Date().toISOString().split("T")[0]}.xlsx"`,
    )
