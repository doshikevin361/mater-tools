...
export async function GET(request: NextRequest) {
  try {
    ...
    if (accounts.length === 0) {
      return NextResponse.json({ success: false, message: "No Facebook accounts found" }, { status: 404 })
    }
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Facebook Accounts")
    ...
    headers.set(
      "Content-Disposition",
      `attachment; filename="facebook-accounts-${new Date().toISOString().split("T")[0]}.xlsx"`,
    )

    return new NextResponse(excelBuffer, { headers })
  } catch (error) {
    console.error("Error generating Facebook accounts Excel file:", error)
