...
async function createFacebookAccount(accountData) {
  ...
  try {
    ...
    await page.type('input[name="firstname"]', accountData.profile.firstName, { delay: 120 })
    await humanWait(1000, 2000)

    // Fill last name
    await page.type('input[name="lastname"]', accountData.profile.lastName, { delay: 120 })
    await humanWait(1000, 2000)

    // Fill email
    await page.type('input[name="reg_email__"]', accountData.email, { delay: 120 })
    await humanWait(1000, 2000)

    // Confirm email
    await page.type('input[name="reg_email_confirmation__"]', accountData.email, { delay: 120 })
    await humanWait(1000, 2000)

    // Fill password
    await page.type('input[name="reg_passwd__"]', accountData.profile.password, { delay: 120 })
    ...
    try {
      ...
      console.log("Gender selection failed:", genderError.message)
    }
    ...
    const successIndicators = [
      currentUrl.includes("facebook.com") && !currentUrl.includes("/reg/"),
...
export async function POST(request) {
  try {
    ...
    await sendNotification(
      userId,
      "Facebook Account Creation Started",
      `Creating ${count} Facebook account${count > 1 ? "s" : ""}. This may take a few minutes...`,
      "info",
    )

    console.log(`🚀 Creating ${count} Facebook accounts...`)

    for (let i = 0; i < count; i++) {
      console.log(`📘 Creating Facebook account ${i + 1}/${count}...`)
      ...
      try {
        ...
        console.log(`🇮🇳 Generated profile: ${profile.fullName}`)
        ...
        if (creationResult.success) {
          successCount++
          console.log(`✅ Facebook account ${i + 1} created: ${creationResult.username}`)
        } else {
          console.log(`❌ Facebook account ${i + 1} failed: ${creationResult.error}`)
        }

        // Send progress notification
        await sendNotification(
          userId,
          "Facebook Account Progress",
          `Account ${i + 1}/${count} ${creationResult.success ? "created successfully" : "failed"}. ${creationResult.success ? `Name: ${creationResult.username}` : `Error: ${creationResult.error}`}`,
        ...
        console.log(`❌ Facebook account ${i + 1} failed: ${error.message}`)
        results.push({
          accountNumber: i + 1,
          success: false,
          platform: "facebook",
    ...
    await sendNotification(
      userId,
      "Facebook Account Creation Completed",
      `Completed creating Facebook accounts! ${successCount}/${count} accounts created successfully.`,
