...
export async function GET(request: NextRequest) {
  try {
    ...
    const campaigns = await db.collection("facebook_campaigns").find({ userId }).sort({ createdAt: -1 }).toArray()
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await db.collection("facebook_campaigns").insertOne(campaign)
    ...
    await db.collection("transactions").insertOne({
      ...
      type: "debit",
      amount: cost,
      description: `Facebook ${type} campaign: ${name}`,
