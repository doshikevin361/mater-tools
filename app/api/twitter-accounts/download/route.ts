...
export async function GET(request: NextRequest) {
  try {
    ...
    if (accounts.length === 0) {
      return NextResponse.json({ success: false, message: "No Twitter accounts found" }, { status: 404 })
    }
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Twitter Accounts")
    ...
    headers.set(
      "Content-Disposition",
      `attachment; filename="twitter-accounts-${new Date().toISOString().split("T")[0]}.xlsx"`,
    )

    return new NextResponse(excelBuffer, { headers })
  } catch (error) {
    console.error("Error generating Twitter accounts Excel file:", error)
