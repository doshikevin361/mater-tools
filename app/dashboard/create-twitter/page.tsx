...
export default function CreateTwitterPage() {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      if (data.success) {
        setNotifications(data.notifications.filter((n) => n.title.includes("Twitter")))
      }
    } catch (error) {
  ...
  const calculateStats = (accountsList: TwitterAccount[]) => {
    const stats = {
  ...
  const createAccounts = async () => {
    if (count < 1 || count > 3) {
      toast.error("Count must be between 1 and 3")
      return
    }
    ...
    setCreating(true)
    try {
      const response = await fetch("/api/twitter-accounts/create", {
        method: "POST",
        headers: {
  ...
  const downloadAccounts = async () => {
    ...
    try {
      const userId = user?._id || "demo_user"
      const response = await fetch(`/api/social-accounts/download?platform=twitter&userId=${userId}`)
      ...
      if (response.ok) {
        ...
        a.href = url
        a.download = `twitter-accounts-${new Date().toISOString().split("T")[0]}.xlsx`
      ...
      <div className="relative z-10 mx-auto max-w-7xl space-y-8 p-6">
        ...
        <div className="text-center py-12">
          <div className="inline-flex items-center gap-3 bg-white/80 backdrop-blur-sm border border-cyan-200 rounded-full px-6 py-3 mb-6">
            ...
            <span className="text-sm font-medium text-cyan-700">Twitter Account Creator</span>
            <Zap className="h-5 w-5 text-yellow-500" />
          </div>

          <h1 className="text-6xl font-bold bg-gradient-to-r from-cyan-600 via-blue-600 to-indigo-600 bg-clip-text text-transparent mb-6">
            Twitter Account Creation
          </h1>

          <p className="text-xl text-slate-600 max-w-3xl mx-auto leading-relaxed">
            Create authentic Twitter accounts with real profiles, advanced anti-detection, and automated verification.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <Card className="bg-white/70 backdrop-blur-sm border border-slate-200/50 hover:bg-white/80 transition-all duration-300 hover:scale-105 hover:shadow-lg">
            ...
            <CardContent>
              ...
              <p className="text-xs text-slate-500 mt-1">Twitter accounts</p>
            </CardContent>
          </Card>
        ...
        {notifications.length > 0 && (
          <Card className="bg-white/80 backdrop-blur-sm border border-slate-200/50 shadow-xl">
            <CardHeader className="bg-gradient-to-r from-cyan-50 to-blue-50 rounded-t-lg border-b border-slate-200/50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <CardDescription className="text-slate-600 text-base mt-1">
                    Real-time updates on Twitter account creation progress
                  </CardDescription>
                </div>
              </div>
            </CardHeader>
        ...
        <Card className="bg-white/80 backdrop-blur-sm border border-slate-200/50 shadow-xl">
          <CardHeader className="bg-gradient-to-r from-cyan-500/10 via-blue-500/10 to-indigo-500/10 rounded-t-lg border-b border-slate-200/50">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-gradient-to-r from-cyan-500 to-blue-500 rounded-lg">
                <Plus className="h-6 w-6 text-white" />
              </div>
              <div>
                <CardTitle className="text-2xl font-bold text-slate-800">Create Twitter Accounts</CardTitle>
                <CardDescription className="text-slate-600 text-base mt-1">
                  Generate authentic Twitter accounts with real profiles and advanced security features
                </CardDescription>
              </div>
            </div>
          </CardHeader>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="space-y-4">
                ...
                <Input
                  ...
                  className="h-14 bg-white border-slate-300 hover:border-cyan-400 focus:border-cyan-500 transition-colors text-slate-700 text-center text-lg font-semibold"
                  placeholder="1-3"
                />
                <p className="text-xs text-slate-500 text-center">Maximum 3 accounts per batch</p>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2 text-sm text-slate-600">
                    <CheckCircle className="h-4 w-4 text-green-500" />
                    <span>Real profiles with names</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm text-slate-600">
                    <CheckCircle className="h-4 w-4 text-green-500" />
              ...
              <div className="space-y-4">
                ...
                <Button
                  ...
                  disabled={creating}
                  className="w-full h-14 bg-gradient-to-r from-cyan-600 via-blue-600 to-indigo-600 hover:from-cyan-700 hover:via-blue-700 hover:to-indigo-700 text-white border-0 shadow-lg font-semibold text-base transition-all duration-300 hover:scale-105 hover:shadow-xl"
                >
                  {creating ? (
                    <>
                ...
                <p className="text-xs text-slate-500 text-center">Estimated time: {count * 3} minutes</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        {/* Accounts List */}
        <Card className="bg-white/80 backdrop-blur-sm border border-slate-200/50 shadow-xl">
          <CardHeader className="bg-gradient-to-r from-slate-50 to-cyan-50 rounded-t-lg border-b border-slate-200/50">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-gradient-to-r from-slate-600 to-cyan-600 rounded-lg">
                  <Twitter className="h-6 w-6 text-white" />
                </div>
                <div>
                  <CardTitle className="text-2xl font-bold text-slate-800">Twitter Accounts ({stats.total})</CardTitle>
                  <CardDescription className="text-slate-600 text-base mt-1">
                    View and manage your created Twitter accounts
                  </CardDescription>
                </div>
              </div>
              ...
              <div className="flex gap-3">
                <Button
                  ...
                  disabled={loading}
                  variant="outline"
                  className="border-cyan-200 text-cyan-700 hover:bg-cyan-50 hover:border-cyan-300 transition-all duration-300 bg-transparent"
                >
                  {loading ? (
                      ...
                      <TableCell colSpan={7} className="text-center text-slate-500 py-16">
                        <div className="flex flex-col items-center">
                          <div className="p-4 bg-slate-100 rounded-full mb-4">
                            <Twitter className="h-12 w-12 text-slate-400" />
                          </div>
                          <p className="text-lg font-medium text-slate-600">No Twitter accounts found</p>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="text-slate-700 font-mono text-sm max-w-[200px] truncate hover:text-cyan-600 cursor-pointer transition-colors bg-slate-50 px-2 py-1 rounded"
                            >
                              {account.email}
                            </button>
                            <Button
                              ...
                              size="sm"
                              onClick={() => copyToClipboard(account.email, "Email")}
                              className="h-6 w-6 p-0 text-slate-400 hover:text-cyan-600 hover:bg-cyan-50 transition-all"
                            >
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="text-slate-700 font-medium hover:text-cyan-600 cursor-pointer transition-colors bg-slate-50 px-2 py-1 rounded"
                            >
                              {account.username}
                            </button>
                            <Button
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="text-slate-700 font-mono text-sm hover:text-cyan-600 cursor-pointer transition-colors bg-slate-50 px-2 py-1 rounded"
                            >
