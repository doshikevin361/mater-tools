...
import { Twitter, Download, Copy, Eye, EyeOff, Bell } from "lucide-react"
...
export default function CreateTwitterPage() {
  ...
  const fetchAccounts = async () => {
    try {
      ...
      const response = await fetch(`/api/social-accounts/list?userId=${user.id}&platform=twitter`)
  ...
  const fetchNotifications = async () => {
    try {
      ...
      if (data.notifications) {
        const twitterNotifications = data.notifications.filter((n: Notification) => n.platform === "twitter")
        setNotifications(twitterNotifications)
      }
    } catch (error) {
  ...
  const createAccounts = async () => {
    ...
    try {
      ...
      if (data.success) {
        toast.success(`Successfully created ${data.summary.success} Twitter accounts!`)
  ...
  const downloadExcel = async () => {
    try {
      ...
      const user = JSON.parse(userData)
      const response = await fetch(`/api/social-accounts/download?userId=${user.id}&platform=twitter`)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-cyan-50 via-white to-blue-50">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-20 left-20 w-24 h-24 bg-cyan-300 rounded-full opacity-20 animate-pulse delay-2000"></div>
      </div>
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 max-w-2xl mx-auto">
            Create multiple Twitter accounts automatically with advanced anti-detection features and real-time
            notifications.
          </p>
        </div>
          ...
          <div className="lg:col-span-2">
            <Card className="bg-white/80 backdrop-blur-sm border-cyan-200">
              <CardHeader>
                ...
                <CardDescription>
                  Generate up to 3 Twitter accounts with Indian profiles and temporary emails
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div>
                  <Label htmlFor="count">Number of Accounts (1-3)</Label>
                  <Input
                    id="count"
                    type="number"
                    min="1"
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Twitter has stricter limits - maximum 3 accounts per batch
                  </p>
                </div>
            ...
            {/* Accounts List */}
            <Card className="mt-6 bg-white/80 backdrop-blur-sm border-cyan-200">
              <CardHeader>
                <CardTitle>Created Accounts ({accounts.length})</CardTitle>
                <CardDescription>Your Twitter accounts with login credentials</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {accounts.map((account) => (
                    <div key={account._id} className="p-4 border border-cyan-200 rounded-lg bg-white/50">
                  ...
                  {accounts.length === 0 && (
                    <div className="text-center py-8 text-gray-500">
                      ...
                      <p>No Twitter accounts created yet</p>
