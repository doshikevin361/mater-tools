...
export default function CreateFacebookPage() {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      if (data.success) {
        setNotifications(data.notifications.filter((n) => n.title.includes("Facebook")))
      }
    } catch (error) {
  ...
  const calculateStats = (accountsList: FacebookAccount[]) => {
    const stats = {
  ...
  const createAccounts = async () => {
    ...
    setCreating(true)
    try {
      const response = await fetch("/api/facebook-accounts/create", {
        method: "POST",
        headers: {
  ...
  const downloadAccounts = async () => {
    ...
    try {
      const userId = user?._id || "demo_user"
      const response = await fetch(`/api/social-accounts/download?platform=facebook&userId=${userId}`)
      ...
      if (response.ok) {
        ...
        a.href = url
        a.download = `facebook-accounts-${new Date().toISOString().split("T")[0]}.xlsx`
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-100 relative overflow-hidden">
      {/* Background Pattern */}
      <div className="absolute inset-0 bg-[url('data:image/svg+xml,%3Csvg width=60 height=60 viewBox=0 0 60 60 xmlns=http://www.w3.org/2000/svg%3E%3Cg fill=none fillRule=evenodd%3E%3Cg fill=%233b82f6 fillOpacity=0.05%3E%3Ccircle cx=30 cy=30 r=2/%3E%3C/g%3E%3C/g%3E%3C/svg%3E')] opacity-40"></div>
      ...
      <div className="relative z-10 mx-auto max-w-7xl space-y-8 p-6">
        ...
        <div className="text-center py-12">
          <div className="inline-flex items-center gap-3 bg-white/80 backdrop-blur-sm border border-blue-200 rounded-full px-6 py-3 mb-6">
            <Facebook className="h-5 w-5 text-blue-600" />
            <span className="text-sm font-medium text-blue-700">Facebook Account Creator</span>
          ...
          <p className="text-xl text-slate-600 max-w-3xl mx-auto leading-relaxed">
            Create authentic Facebook accounts with real profiles, advanced anti-detection, and automated verification.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <Card className="bg-white/70 backdrop-blur-sm border border-slate-200/50 hover:bg-white/80 transition-all duration-300 hover:scale-105 hover:shadow-lg">
            ...
            <CardContent>
              ...
              <p className="text-xs text-slate-500 mt-1">Facebook accounts</p>
            </CardContent>
          </Card>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <CardDescription className="text-slate-600 text-base mt-1">
                    Real-time updates on Facebook account creation progress
                  </CardDescription>
                </div>
              </div>
            </CardHeader>
        ...
        <Card className="bg-white/80 backdrop-blur-sm border border-slate-200/50 shadow-xl">
          <CardHeader className="bg-gradient-to-r from-blue-500/10 via-indigo-500/10 to-purple-500/10 rounded-t-lg border-b border-slate-200/50">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-gradient-to-r from-blue-500 to-indigo-500 rounded-lg">
                <Plus className="h-6 w-6 text-white" />
              </div>
              <div>
                <CardTitle className="text-2xl font-bold text-slate-800">Create Facebook Accounts</CardTitle>
                <CardDescription className="text-slate-600 text-base mt-1">
                  Generate authentic Facebook accounts with real profiles and advanced security features
                </CardDescription>
              </div>
            </div>
          </CardHeader>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="space-y-4">
                ...
                <Input
                  ...
                  className="h-14 bg-white border-slate-300 hover:border-blue-400 focus:border-blue-500 transition-colors text-slate-700 text-center text-lg font-semibold"
                  placeholder="1-5"
                />
              ...
              <div className="space-y-4">
                ...
                <Button
                  ...
                  disabled={creating}
                  className="w-full h-14 bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 hover:from-blue-700 hover:via-indigo-700 hover:to-purple-700 text-white border-0 shadow-lg font-semibold text-base transition-all duration-300 hover:scale-105 hover:shadow-xl"
                >
                  {creating ? (
                    <>
          ...
          <CardHeader className="bg-gradient-to-r from-slate-50 to-blue-50 rounded-t-lg border-b border-slate-200/50">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <CardTitle className="text-2xl font-bold text-slate-800">Facebook Accounts ({stats.total})</CardTitle>
                  <CardDescription className="text-slate-600 text-base mt-1">
                    View and manage your created Facebook accounts
                  </CardDescription>
                </div>
              </div>
              ...
              <div className="flex gap-3">
                <Button
                  ...
                  disabled={loading}
                  variant="outline"
                  className="border-blue-200 text-blue-700 hover:bg-blue-50 hover:border-blue-300 transition-all duration-300 bg-transparent"
                >
                  {loading ? (
                      ...
                      <TableCell colSpan={7} className="text-center text-slate-500 py-16">
                        <div className="flex flex-col items-center">
                          <div className="p-4 bg-slate-100 rounded-full mb-4">
                            <Facebook className="h-12 w-12 text-slate-400" />
                          </div>
                          <p className="text-lg font-medium text-slate-600">No Facebook accounts found</p>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="text-slate-700 font-mono text-sm max-w-[200px] truncate hover:text-blue-600 cursor-pointer transition-colors bg-slate-50 px-2 py-1 rounded"
                            >
                              {account.email}
                            </button>
                            <Button
                              ...
                              size="sm"
                              onClick={() => copyToClipboard(account.email, "Email")}
                              className="h-6 w-6 p-0 text-slate-400 hover:text-blue-600 hover:bg-blue-50 transition-all"
                            >
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="text-slate-700 font-medium hover:text-blue-600 cursor-pointer transition-colors bg-slate-50 px-2 py-1 rounded"
                            >
                              {account.username}
                            </button>
                            <Button
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="text-slate-700 font-mono text-sm hover:text-blue-600 cursor-pointer transition-colors bg-slate-50 px-2 py-1 rounded"
                            >
